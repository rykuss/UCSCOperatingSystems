...
// Our sleeping Thread List
struct list tse_list;
...
timer_sleep (int64_t ticks) 
{
  ...
  //printf("\nStart = %d ; Ticks = %d ; wake_up = %d \n", start, ticks, cur->wake_up);

  list_push_back(&tse_list, &(cur->sleepPurposedElem));
...
timer_interrupt (struct intr_frame *args UNUSED)
{
  ...
  for (e = list_begin(&tse_list); e != list_end(&tse_list); e = list_next(e))
  {
    struct thread *f = list_entry (e, struct thread, sleepPurposedElem);
    if((f->wake_up) <= ticks)
    {
      f->wake_up = -5000;
      thread_unblock(f);
